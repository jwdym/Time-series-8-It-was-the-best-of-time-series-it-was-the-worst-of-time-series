...
A trend model with limited flexibility will help ensure that we preserve the underlying variation in the time series. To form these models, careful thought needs to be put into describing the trend to avoid removing variation that we are interested in. Using more complicated terms, such as higher order polynomial terms or splines, can lead to over fitting the data, so there should be good reason to include higher order terms.
...
The sample size, or length of time series, for the Douglas Fir Trees, is 80 years for 8 trees. Thus, we have 640 observations in the Douglas Fir Data.

The sample size for the Western Pine is 80 years of observations on 10 trees, thus we have 800 total observations in the Western Pine data.

We would have plotted cutoff values of $\frac{2}{\sqrt{n}}$ so for the Douglas Fir trees we would have cutoff values at 0.079 and 0.071 for the Douglas Fir trees and White Pine trees, respectively.
...
_We simulate the time series with SACF plots below. Compared to the plots in Figure 3 (the lower panel since we're interested in Western Pines), we can see that the first and second lags are quite large in the fake plot; this is very similar to the plot that the authors produced. Moreover, while we see the occasional false spike (one occurs at the 19th spike), most of the larger lags appear to be no different from 0. For the most part, we see very similar results from the simulated data as compared to their data._
```{r}
#looks at the Western Pine Trees
set.seed(123456)
simz <- arima.sim(n = 800, list(ma=c(2,0)),sd = 0.4 )
plot(simz, main = "Fake Time Series", ylab = "Simulated Response", xlab = "Time")
acf(simz, main = "Fake ACF Plot") #SACF plot
