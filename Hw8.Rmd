...
We would have plotted cutoff values of_ $\frac{2}{\sqrt{n}}$ _so for the Douglas Fir trees we would have cutoff values at 0.079 and 0.071 for the Douglas Fir trees and White Pine trees, respectively.
...
_The published results for each AR(2) model are shown on page 1097. When we take the roots of the AR(2) polynomial for the k, we get exactly one _
```{r}
polyroot(c(.37,.15))
```
